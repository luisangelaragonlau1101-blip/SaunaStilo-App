...
replace('lib/screens/mensajes_equipo_screen.dart', "final result = await FilePicker.platform.pickFiles(allowMultiple: true, withData: true); for (final file in result?.files ?? <PlatformFile>[]) { if (file.bytes == null) { _notice('No se pudo leer ${file.name}.'); continue; } _addFile(file.name, file.bytes!); }", "final files = await FilePicker.pickFiles(allowMultiple: true); for (final file in files) { if (!mounted) return; if (await file.length() > 16 * 1024 * 1024) { _notice('Cada archivo debe pesar menos de 16 MB.'); continue; } final bytes = await file.readAsBytes(); if (!mounted) return; _addFile(file.name, bytes); }")
