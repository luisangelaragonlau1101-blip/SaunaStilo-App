...
import copy,hashlib,importlib.util,json,re,zipfile
...
with zipfile.ZipFile(files[0]) as src:
    ...
    with zipfile.ZipFile('android-report/un-aligned.apk','w') as dst:
        for e in src.infolist():
            ...
            # writestr mutates ZipInfo.header_offset. Never reuse source entry objects.
            dst.writestr(copy.copy(e),content)
    with zipfile.ZipFile('android-report/un-aligned.apk') as out:
        ...
        for e in src.infolist():
            ...
            assert out.read(e.filename)==src.read(e.filename), e.filename
