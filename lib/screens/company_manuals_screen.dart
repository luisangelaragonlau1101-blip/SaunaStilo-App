...
class _ManualEditorState extends State<CompanyManualEditor>{
 ...
 Future<void> _import()async{setState(()=>_busy=true);try{final result=await FilePicker.platform.pickFiles(type:FileType.custom,allowedExtensions:['pdf','txt','md'],withData:true);if(result==null)return;final file=result.files.single;if(file.bytes==null||file.size>2*1024*1024)throw StateError('Selecciona un archivo legible de hasta 2 MB.');final r=await service.call('manual-import',{'filename':file.name,'base64':base64Encode(file.bytes!)});if(mounted){content.text=r['content'];setState((){_filename=file.name;_reviewed=false;_error=null;});}}catch(e){if(mounted)setState(()=>_error=CompanyLearningService.message(e));}finally{if(mounted)setState(()=>_busy=false);}}
