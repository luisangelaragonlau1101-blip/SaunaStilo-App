...
class VozAdministracionScreen extends StatefulWidget {
  ...
  @override
  State<VozAdministracionScreen> createState() => _VozAdministracionScreenState();
}
...
class _VozAdministracionScreenState extends State<VozAdministracionScreen> {
  ...
  bool get _locked => _busy || _recording != null;
  ...
  Future<void> _loadStatus() async {
    if (!_admin || _checkingStatus) return;
    setState(() => _checkingStatus = true);
    try {
      final status = await _voice.status();
      if (mounted) setState(() { _status = status; _statusError = null; });
    } catch (error) {
      if (mounted) setState(() {
        _status = null;
        _statusError = error is CustomVoiceException ? error.message : 'No pude consultar el estado del servicio de voz.';
      });
    } finally { if (mounted) setState(() => _checkingStatus = false); }
  }
  ...
  Future<void> _previewRecording(Uint8List bytes) async {
    ...
    } catch (_) { if (mounted) setState(() => _message = 'No se pudo escuchar la muestra. Revisa el sonido del dispositivo y vuelve a tocar Escuchar.'); }
  }
  Future<void> _startRecording(_VoiceSlot slot) async {
    ...
    } catch (_) { if (mounted) setState(() { _busy = false; _message = 'No se inició la grabación. Revisa el permiso de micrófono y vuelve a tocar Grabar.'; }); }
  }
  Future<void> _stopRecording() async {
    ...
    try {
      final Uint8List wav;
      if (capture.isWebCapture) { wav = await capture.finishCapture(); }
      else {
    ...
    } catch (_) { if (mounted) setState(() => _message = 'No se guardó la muestra. Graba de 3 a 10 segundos y revisa que el micrófono esté permitido.'); }
    finally { if (mounted) setState(() => _busy = false); }
  }
  Future<void> _enroll() async {
    final consent = _consentAudio;
    final reference = _referenceAudio;
    if (consent == null || reference == null || _locked) return;
    setState(() { _busy = true; _message = 'Enviando las grabaciones al servicio de voz; no se publican en archivos compartidos…'; });
    try {
      final status = await _voice.enroll(consentBase64: base64Encode(consent), referenceBase64: base64Encode(reference), languageCode: 'es-US');
      if (!mounted) return;
      setState(() { _status = status; _statusError = null; _message = status.message; });
    } catch (error) { if (mounted) setState(() => _message = error is CustomVoiceException ? error.message : 'No se pudo crear la voz. Revisa la conexión y la configuración de Google Cloud.'); }
    finally { if (mounted) setState(() => _busy = false); }
  }
  Future<void> _testVoice() async {
    ...
    } catch (error) { if (mounted) setState(() => _message = error is CustomVoiceException ? error.message : 'No pude reproducir la prueba de voz.'); }
    finally { if (mounted) setState(() => _busy = false); }
  }
  Future<void> _toggleEnabled(bool enabled) async {
    ...
    try {
      ...
      if (mounted) setState(() { _status = status; _statusError = null; _message = status.message; });
    } catch (error) { if (mounted) setState(() => _message = error is CustomVoiceException ? error.message : 'No pude cambiar el estado de la voz.'); }
    finally { if (mounted) setState(() => _busy = false); }
  }
  Uint8List _createWav(Uint8List pcm, {required int sampleRate, required int channels}) {
    ...
    void writeText(int offset, String value) {
      ...
      for (var index = 0; index < data.length; index++) { header.setUint8(offset + index, data[index]); }
    }
    writeText(0, 'RIFF'); header.setUint32(4, 36 + pcm.length, Endian.little);
    writeText(8, 'WAVE'); writeText(12, 'fmt '); header.setUint32(16, 16, Endian.little);
    header.setUint16(20, 1, Endian.little); header.setUint16(22, channels, Endian.little);
    header.setUint32(24, sampleRate, Endian.little); header.setUint32(28, byteRate, Endian.little);
    header.setUint16(32, blockAlign, Endian.little); header.setUint16(34, bitsPerSample, Endian.little);
    writeText(36, 'data'); header.setUint32(40, pcm.length, Endian.little);
    final builder = BytesBuilder(copy: false)..add(header.buffer.asUint8List())..add(pcm);
