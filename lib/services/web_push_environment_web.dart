// Browser-only feature checks. Native builds keep using the conditional stub.
// Static interop replaces dart:js_util, which is not available in current Dart.
...
external String get _protocol;
@JS('window.location.hostname')
external String get _hostname;
@JS('window.Notification')
external JSAny? get _notification;
@JS('navigator.serviceWorker')
external JSAny? get _serviceWorker;
@JS('window.PushManager')
external JSAny? get _pushManager;
...
String? pushEnvironmentProblem() {
  ...
  final ios = RegExp(r'iPad|iPhone|iPod').hasMatch(ua) ||
      (ua.contains('Macintosh') && (_maxTouchPoints ?? 0) > 1);
  final standalone = _matchMedia('(display-mode: standalone)').matches ||
      _iosStandalone == true;
  if (ios && !standalone) {
  ...
  if (_protocol != 'https:' && _hostname != 'localhost' && _hostname != '127.0.0.1') {
    return 'Los avisos necesitan la dirección HTTPS de la aplicación.';
  }
  if (_notification == null || _serviceWorker == null || _pushManager == null) {
