...
class _ConexionPanelState extends State<ConexionPanel> {
  ...
  Widget build(BuildContext context) => Container(
    ...
    decoration: BoxDecoration(
      gradient: const LinearGradient(colors: [Color(0xFF190C12), Color(0xFF0C0C0C)]),
      borderRadius: BorderRadius.circular(24),
      border: Border.all(color: const Color(0xFF3A2430)),
    ),
    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      const Row(children: [Icon(Icons.hub_outlined, color: Color(0xFFB7FF2A)), SizedBox(width: 10),
        Expanded(child: Text('Tu centro de conexión', style: TextStyle(fontSize: 17, fontWeight: FontWeight.w700, color: Colors.white)))]),
      const SizedBox(height: 12),
      Semantics(liveRegion: true, child: Text(_status, style: const TextStyle(color: Color(0xFFCCCCCC), height: 1.5))),
      ...
      Wrap(spacing: 10, runSpacing: 10, children: [
        FilledButton.icon(onPressed: _pushBusy ? null : _activate,
          ...
          style: FilledButton.styleFrom(minimumSize: const Size(0, 48), backgroundColor: const Color(0xFFB7FF2A), foregroundColor: const Color(0xFF080808))),
        OutlinedButton.icon(onPressed: _aiBusy ? null : _testAi,
          ...
          style: OutlinedButton.styleFrom(minimumSize: const Size(0, 48), foregroundColor: const Color(0xFFB7FF2A))),
      ]),
      const SizedBox(height: 10),
      const Text('El sonido y los avisos con pantalla bloqueada dependen de los permisos y del modo Enfoque del teléfono.', style: TextStyle(fontSize: 12, color: Color(0xFFAAAAAA), height: 1.4)),
    ]),
  );
}
