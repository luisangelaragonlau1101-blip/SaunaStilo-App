...
printf '\nProyecto: %s\n' "$PROJECT"
printf 'Esta activación publica asistencia, recordatorios, tareas del maestro, IA y envío push. El uso puede generar cargos en Google Cloud.\n'
printf 'No cambia tu plan de facturación ni usuarios. Actualiza reglas para que el maestro asigne tareas solo en sus proyectos. No crea registros de asistencia de prueba.\n'
...
gcloud services enable cloudfunctions.googleapis.com run.googleapis.com cloudbuild.googleapis.com artifactregistry.googleapis.com eventarc.googleapis.com pubsub.googleapis.com aiplatform.googleapis.com texttospeech.googleapis.com fcm.googleapis.com cloudscheduler.googleapis.com --project="$PROJECT"
npm --prefix functions ci --ignore-scripts
node --test tests/*.test.cjs
firebase deploy --project "$PROJECT" --only 'functions:saunaAssistant,functions:saunaAssistantV2,functions:sendSaunaStiloNotification,functions:getAdminVoiceStatus,functions:enrollAdminVoice,functions:setAdminVoiceEnabled,functions:synthesizeAdminVoice,functions:updateAttendance,functions:reminderEntrada,functions:reminderComida,functions:reminderSalida,firestore:rules'
firebase functions:list --project "$PROJECT"
printf '\nDespliegue terminado. Abre Inicio > Jornada para verificar entrada y salida; Online Smart tiene su acceso propio. Prueba también Guía y los avisos con dos teléfonos.\n'
