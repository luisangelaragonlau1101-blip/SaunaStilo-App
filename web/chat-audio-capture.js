...
window.createSaunaChatAudioCapture = () => {
  let session = null;
  let starting = false;
  let generation = 0;
  ...
  return {
    async start(maxSeconds = 180) {
      maxSeconds = Math.max(1, Math.min(180, Number(maxSeconds) || 180));
      ...
      try {
        ...
        const s = {stream, context, rate: context.sampleRate, chunks: [], length: 0, max: Math.floor(context.sampleRate * Math.max(1, Math.min(180, maxSeconds))), closed: false};
        session = s;
    ...
    async stop() {
      ...
      await clean(s);
      if (s.length < s.rate * 1) throw new Error('Graba entre 1 y 180 segundos.');
      ...
      const length = Math.min(4320000, Math.round(s.length * 24000 / s.rate));
