...
function buildPushPayload({ notificationId, data, now = Date.now() }) {
  ...
  const isPersonal = type === 'aviso_personal';
  ...
  const ttl = liveCall ? Math.max(1, Math.ceil((created + 60000 - now) / 1000)) : (isAlarm || isPersonal || type.startsWith('asistencia_') ? 900 : 86400);
  ...
  const extra = {};
  for (const [target, source] of Object.entries({ route: 'ruta', projectId: 'proyectoId', conversationId: 'conversacionId', activityId: 'actividadId' })) {
  ...
  return {
    ...
    android: {
      ...
      notification: {
        ...
        sound: 'default', priority: liveCall || isAlarm || isPersonal ? 'max' : 'high',
        tag: notificationId,
        sticky: liveCall || isAlarm || isPersonal,
      },
    },
    apns: {
    ...
    webpush: {
      ...
      notification: {
        ...
        requireInteraction: liveCall || isAlarm || isPersonal, renotify: liveCall || isAlarm || isPersonal, silent: false,
